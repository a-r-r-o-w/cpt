...
def _try_typecast(
    value: typing.Any, to_type: typing.Type, default: typing.Any = None
) -> typing.Any:
    try:
...
class ProblemStatistic(CodeforcesObject):
    def __init__(self, contest_id: int, index: str, solved_count: int):
        self.contest_id = contest_id
        self.index = index
...
class RecentAction(CodeforcesObject):
    def __init__(self, time_seconds: int, blog_entry: BlogEntry, comment: Comment):
...
def problemstatistic_parse(
    problemstatistics: typing.List[dict],
) -> typing.List[ProblemStatistic]:
    ...
    for problemstatistic in problemstatistics:
        ...
        problemstatistic_list.append(ProblemStatistic(contest_id, index, solved_count))

    return problemstatistic_list


def problemresult_parse(
    problemresults: typing.List[dict],
) -> typing.List[ProblemResult]:
    problemresult_list = []

    for problemresult in problemresults:
        penalty = _try_typecast(problemresult.get("penalty"), int, "NA")
        best_submission_time_seconds = _try_typecast(
            problemresult.get("bestSubmissionTimeSeconds"), int, "NA"
        )
...
def recentaction_parse(recentactions: typing.List[dict]) -> typing.List[RecentAction]:
    ...
    for recentaction in recentactions:
        ...
        recentaction_list.append(RecentAction(time_seconds, blog_entry, comment))
...
def contest_parse(contests: typing.List[dict]) -> typing.List[Contest]:
    ...
    for contest in contests:
        ...
        relative_time_seconds = _try_typecast(
            contest.get("relativeTimeSeconds"), int, "NA"
        )
...
def ratingchange_parse(ratingchanges: typing.List[dict]) -> typing.List[RatingChange]:
    ...
    for ratingchange in ratingchanges:
        ...
        rating_update_time_seconds = _try_typecast(
            ratingchange.get("ratingUpdateTimeSeconds"), int, 0
        )
        contest_name = ratingchange.get("contestName")
        handle = ratingchange.get("handle")

        ratingchange_list.append(
            RatingChange(
                contest_id,
                contest_name,
                handle,
...
def ranklistrow_parse(ranklistrows: typing.List[dict]) -> typing.List[RanklistRow]:
    ...
    for ranklistrow in ranklistrows:
        ...
        last_submission_time_seconds = _try_typecast(
            ranklistrow.get("lastSubmissionTimeSeconds"), int, -1
        )
